        ...
        if settings[0][0]:
            ...
            if a.get_role(settings[0][4]) == None:
                ...
                for i in a.roles:
                    if i.name == 'Faction Member':
                        found = True
                        if ch.permissions_for(a.me).send_messages:
                            embed = discord.Embed(title = ':gear: Settings - Role - Value', description = f'Could not find previously set role for __value module__.\nTo overcome this issue, the {i.mention} has been selected.\n\nTo change the role for __value module__,\n```{prefix}settings role value <mention the role>```', color = discord.Colour.blue())
                            embed.set_footer(text = f'{client.user.name} | {datetime.datetime.utcnow().strftime(dateformat)}')
                            await ch.send(embed = embed)
                        settings[0][4] = i.id
                ...
                if not found:
                    if a.me.guild_permissions.manage_roles:
                        ...
                        if ch.permissions_for(a.me).send_messages:
                            ...
                            settings[0][4] = r.id
                            change = True
                            if ch.permissions_for(a.me).manage_permissions:
                                await ch.set_permissions(r, read_messages = True, send_messages = True)
                            else:
                                embed = discord.Embed(title = ':warning: Insufficient permissions.', description = f'Unable to set permissions for {r.mention} role in {ch.mention} due to insufficient permissions. Please give sufficient permissions for the proper functioning of the bot.', color = discord.Colour.red())
                                await ch.send(embed = embed)
                    else:
                        if ch.permissions_for(a.me).send_messages:
                            embed = discord.Embed(title = ':warning: Insufficient Permissions', description = 'I noticed that the role for value module has not been set or has been deleted. Normally the bot would have automatically made a new role but this action could not be performed due to insufficient permissions.', color = discord.Colour.red())
                            await ch.send(embed = embed)
        if settings[1][0]:
            if client.get_channel(settings[1][5]) == None:
                found = False
                    ...
                    if i.name == 'Faction Member':
                        ...
                        if ch.permissions_for(a.me).send_messages:
                            ...
                            if ch.permissions_for(a.me).manage_permissions:
                                ...
                                embed = discord.Embed(title = ':warning: Insufficient Permissions', description = f'Unable to set permissions for {i.mention} in {ch.mention} due to insufficient permisssions.', color = discord.Colour.red())
                                await ch.send(embed = embed)
                        settings[1][6] = i.id
                        change = True
                    ...
                    if a.me.guild_permissions.manage_roles:
                        ...
                        if ch.permissions_for(a.me).send_messages:
                            ...
                            if ch.permissions_for(a.me).manage_permissions:
                                ...
                                embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = 'Unable to set permissions for {r.mention} in {ch.mention} due to insufficient permissions.', color = discord.Colour.red())
                                await ch.send(embed = embed)
                        settings[1][6] = r.id
                        change = True
                    else:
                        if ch.permissions_for(a.me).send_messages:
                            embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = 'I noticed that the role for the walls module has not been set or has been deleted. Normally the bot would have created a new role automatically but this action could not be performed due to insufficient permissions.', color = discord.Colour.red())
                            await ch.send(embed = embed)
                    ...
                    if i.name == 'Faction Member':
                        ...
                        if ch.permissions_for(a.me).send_messages:
                            ...
                            if ch.permissions_for(a.me).manage_permissions:
                                ...
                                embed = discord.Embed(title = ':warning: Insufficient Permissions', description = f'Could not set permissions for {i.mention} in {ch.mention} due to sufficient permissions.', color = discord.Colour.red())
                                await ch.send(embed = embed)
                        settings[2][6] = i.id
                        change = True
                    ...
                    if a.me.guild_permissions.manage_roles:
                        ...
                        if ch.permissions_for(a.me).send_messages:
                            ...
                            if ch.permissions_for(a.me).manage_permissions:
                                ...
                                embed = discord.Embed(title = ':warning: Insufficient Permissions', description = f'Could not set permissions for {r.mention} in {ch.mention} due to insufficient permissions.', color = discord.Colour.red())
                                await ch.send(embed = embed)
                        settings[2][6] = r.id
                        change = True
                    else:
                        if ch.permissions_for(a.me).send_messages:
                            embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = 'I noticed that the role for the buffers module has not been set or has been deleted. Normally the bot would have created a new role automatically but this action could not be performed due to insufficient permissions.', color = discord.Colour.red())
                            await ch.send(embed = embed)
            if settings[2][3] == None:
                settings[2][3] = datetime.datetime.utcnow().strftime(dateformat)
                change = True
            ...
            if i.name == 'Faction Member':
                ...
                if ch.permissions_for(a.me).send_messages:
                    ...
                    if ch.permissions_for(a.me).manage_permissions:
                        ...
                        embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = f'Could not set permissions for {i.mention} in {ch.mention} due to insufficient permissions.', color = discord.Colour.red())
                        await ch.send(embed = embed)
                settings[0][4] = i.id
                change = True
            ...
            if a.me.guild_permissions.manage_roles:
                ...
                if ch.permissions_for(a.me).send_messages:
                    ...
                    if ch.permissions_for(a.me).manage_permissions:
                        ...
                        embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = f'Could not set permissions for {r.mention} in {ch.mention} due to insufficient permissions.', color = discord.Colour.red())
                        await ch.send(embed = embed)
                settings[0][4] = r.id
                change = True
            else:
                if ch.permissions_for(a.me).send_messages:
                    embed = discord.Embed(title = ':warning: Insufficient Permissions', descripton = 'I noticed that the roole for the value module has not been set or has been deleted. Normally the bot would have created a new role automatically but this action could not be performed due to insufficient permissions.', color = discord.Colour.red())
                    await ch.send(embed = embed)
        with open(os.path.join(settingsDir, str(a.id)+'.txt'), 'w+') as file:
            file.write(str(settings))
